...
{
    public static class NumberToWordConversion
    {
        public static string NumberToCurrencyText(decimal number)
        {
            ...
            number = decimal.Round(number, 2, 0);
            ...
            // For amounts of zero dollars but have cents show nothing, if no dollar and no cent show 'Zero Dollar...'
            ...
            if (arrNumber.Length > 1)
            {
                ...
                if (fractionPart != 0)
                {
                    wordNumber += (wordNumber.Length > 0 ? " and " : "") + strFarctionPart + (fractionPart == 1 ? " Cent" : " Cents");
                }
                else if(wordNumber.Length == 0)
                {
                    wordNumber = strFarctionPart + " Cent";
                }
            }
           

            return wordNumber.ToUpper(CultureInfo.InvariantCulture);
        }
